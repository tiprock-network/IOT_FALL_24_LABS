...
roomRoute.post('/rooms', async(req,res) => {
    ...
    try {
        ...
        const qry  = "INSERT INTO rooms(name) VALUES ($1) RETURNING *"
        const result = await client.query(qry,[rmName])
...
roomRoute.get('/rooms/:id', async (req,res) => {
    ...
    try {
        //Implement Code for database here
        const qry  = "SELECT * FROM rooms WHERE id = $1;"
...
roomRoute.patch('/rooms/:id/light', async(req,res) => {
    ...
    try {
        ...
        const result = await client.query(qry,[lightStatus=="on"?true:false,id])
